...
export class GeminiSessionApi {
     ...
     */
    async ask(prompt, { token = null, cursor = ["", "", ""], signal } = {}, retrying = false) {
        ...
        if (response.status !== 200) {
            const responseText = await this.utils?.noThrow?.(() => response.text(), null) || await response.text();
        ...
        let l, c, u, p;
        try {
            // Gemini returns an XSSI prefix like ")]}'" followed by multiple lines.
            const raw = await response.text();
            const cleaned = raw.replace(/^\)\]\}'\s*\n?/, '').trim();
            const firstJsonLine = cleaned.split('\n').find(line => line.trim().startsWith('['));
            if (!firstJsonLine) throw new Error('No JSON line detected in response');
            l = JSON.parse(firstJsonLine);
        }
        catch (e) {
        ...
        return (async (...args) => {
            try {
                ...
                if (this.isOwnError(e))
                    ...
                    err = this._createError("unknown", e.message);
                if (err.details)
...
export class GeminiProviderController {
    ...
    async init() {
        ...
        if (typeof BusController !== 'undefined') {
            BusController.on('gemini-provider.ask', this._handleAskRequest.bind(this));
            BusController.on('gemini-provider.fetchToken', this._handleFetchTokenRequest.bind(this));
        }
        this.initialized = true;
    }
    async _handleAskRequest(payload) {
        return await this.api.ask(payload.prompt, payload.options || {}, payload.retrying || false);
    }
